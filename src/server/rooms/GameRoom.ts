...
export class GameRoom extends Room<GameState> {
        ...
        this.state.players.forEach((player) => {
            ...
            if (player.mana < player.maxMana) {
                player.mana = Math.min(
                    player.maxMana,
                    player.mana + (GAME_CONSTANTS.MANA_REGEN * deltaTime / 1000)
                );
            }
            ...
            if (player.stamina < player.maxStamina) {
                player.stamina = Math.min(
                    player.maxStamina,
                    player.stamina + (GAME_CONSTANTS.STAMINA_REGEN_STATIONARY * deltaTime / 1000)
                );
            }
    ...
    private handleMove(client: Client, data: { x: number; y: number; z: number; rotationY?: number }) {
        const player = this.state.players.get(client.sessionId);
    ...
    private handleAbility(client: Client, data: { abilityId: string; targetId?: string }) {
        const player = this.state.players.get(client.sessionId);
